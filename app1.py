...
# KTP Document
@app.route('/ocr/local/ktp', methods=['POST'])
def ocr_ktp():
    # Get image path from request data
    image_path = request.form['image_path']
    image2_path = request.form['image2_path']
    head, tail = os.path.split(image_path)
    selfie_folder = "input_diff_dir_doc/dataset/Selfie/"
    selfiepath = os.path.join(selfie_folder, tail)
        

    # check if file is an image
    ocr = KTPOCR(image_path, image2_path)
