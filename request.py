...
def ktp_ocr(dir_path, dir2_path, output_path, reset_data):
    url = 'http://localhost:5000/ocr/local/ktp'

        # Send a POST request to the endpoint with the image path
    response = requests.post(url, data={'image_path': dir_path, 'image2_path': dir2_path})
    # Print the response text
    print(response.content)
    ...
    jsonFile.write(str(response.content))
    jsonFile.close()
...
ktp_ocr(dir_path='input_dirr_dif_doc/dataset/KTP/7.png', dir2_path='input_dirr_dif_doc/dataset/Selfie/7.png',output_path = 'output_dirr_dif_doc/diffktp_doc.json', reset_data=True)
