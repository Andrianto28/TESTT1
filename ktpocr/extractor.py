...
class KTPOCR(object):
    ...
    def nik_extract(self, word):
        ...
        for letter in word:
            if letter in word_dict:
                ...
                res += letter
        return res
    def get_nik(self, image):
        cong = r"--oem 3 --psm 6 outputbase digits"
        boxes = pytesseract.image_to_data((self.threshed),config=cong, lang='ind')
        k = 0
        res = ""
        tgl = ""
        for x, b in enumerate(boxes.splitlines()):
            if x!= 0:
                b = b.split()
                if len(b) ==12:
                    if k < 20 and (len(b[11]) > 15):
                        res = (b[11])
                    if len(b[11]) == 10 and ("1" in b[11]):
                        tgl = b[11]
                    k+=1
        return res, tgl
    ...
    def extract(self, extracted_result):
        ...
        # Get NIK
        self.result.nik, A = self.get_nik(self.image)
        self.result.nik = self.nik_extract(self.result.nik.replace(" ", ""))
        for word in extracted_result.split("\n"):
            ...
            if "Nama" in word:
                ...
                self.result.nama = re.sub(r'[^\w]', ' ', self.result.nama).strip()
                if len(self.result.nama) < 2:
                    self.result.nama = word[-2].replace('Nama ','').strip()
                continue
            ...
            if "Lahi" in word:
                ...
                if len(self.result.tanggal_lahir) < 8:
                    self.result.tanggal_lahir = A
                    self.result.tempat_lahir = word1.replace(self.result.tanggal_lahir,"").split(" ")
                    print(self.result.tempat_lahir)
                    self.result.tempat_lahir = self.result.tempat_lahir[-2].replace(",","")
                    continue
            ...
            if 'Darah' in word:
                ...
                if "LAKI" in self.result.jenis_kelamin:
            ...
            if 'Alamat' in word:
                ...
                self.result.alamat = re.sub('[a-z]', '', (re.sub(r'[^\w]', ' ', self.result.alamat).strip()))
            ...
            try:
                if "Desa" in word:
                    ...
                    self.result.kelurahan_atau_desa = re.sub(r'[^\w]', ' ', self.result.kelurahan_atau_desa.replace(":","").replace("-","").strip())
                    self.result.kelurahan_atau_desa = re.sub('[a-z]', '',self.result.kelurahan_atau_desa).strip()
